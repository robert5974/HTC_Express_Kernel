...
static void mmc_blk_put(struct mmc_blk_data *md)
{
	...
	if (md->usage == 0) {
		...
		printk(KERN_INFO "%s: new dev_use %lu clear devidx %d,  MMC_NUM_MINORS %d\n", md->disk->disk_name, *dev_use, devidx, MMC_NUM_MINORS);
		put_disk(md->disk);
		kfree(md);
	} else {
		int devidx = md->disk->first_minor >> MMC_SHIFT;
		printk(KERN_INFO "%s: md->usage > 0, Keep minor status dev_use %lu devidx %d,  MMC_NUM_MINORS %d\n", md->disk->disk_name, *dev_use, devidx, MMC_NUM_MINORS);
	}
	mutex_unlock(&open_lock);
}
...
#ifdef CONFIG_MMC_PERF_PROFILING
		emmc_diff = ktime_sub(ktime_get(), emmc_start);
		if (ktime_to_us(emmc_diff) > 150000)
			printk(KERN_DEBUG "%s:(%s) check emmc resume  time=%lld ms\n", __func__, current->comm, ktime_to_ms(emmc_diff));
#endif
	}
	...
	do {
		...
		if (mmc_card_sd(card)) {
			...
			if (ktime_to_us(diff) > 150000)
				printk(KERN_DEBUG "%s:(%s)finish cmd(%d) time=%lld ms\n", __func__, current->comm, brq.cmd.opcode, ktime_to_ms(diff));
		}
#endif
		/*
		...
		if (!mmc_host_is_spi(card->host) && rq_data_dir(req) != READ) {
			int i = 0;
			int sleepy = (mmc_card_sd(card) || mmc_card_mmc(card)) ? 1 : 0;
			unsigned int msec = 0;
			unsigned long delay = jiffies + HZ;
			err = 0;
			do {
		...
		if (mmc_card_sd(card)) {
			...
			if (ktime_to_us(diff) > 300000)
				printk(KERN_DEBUG "%s: ---(%s) start sector=%d, size %d, total time=%lld ms\n", __func__, current->comm, brq.cmd.arg, blk_rq_sectors(req) , ktime_to_ms(diff));
		}
#endif
#if 0
...
static struct mmc_blk_data *mmc_blk_alloc(struct mmc_card *card)
{
	...
	if (devidx >= MMC_NUM_MINORS) {
		printk(KERN_ERR "mmc: Failed to get device minor number, dev_use %lu, devidx %d, MMC_NUM_MINORS %d\n", *dev_use, devidx, MMC_NUM_MINORS);
		return ERR_PTR(-ENOSPC);
	}

	__set_bit(devidx, dev_use);
	printk(KERN_INFO "mmc: Set dev_use %lu, devidx %d, MMC_NUM_MINORS %d\n", *dev_use, devidx, MMC_NUM_MINORS);
