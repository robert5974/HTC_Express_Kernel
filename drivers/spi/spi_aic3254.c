...
static int spi_transaction(struct spi_message *msg)
{
	DECLARE_COMPLETION_ONSTACK(spi_done);
	int status;

	msg->complete = spi_complete;
	msg->context = &spi_done;

	spin_lock_irq(&spinlock);
	if (codec_dev == NULL)
		status = -ESHUTDOWN;
	else
		status = spi_async(codec_dev, msg);
	spin_unlock_irq(&spinlock);

	if (status == 0) {
		wait_for_completion(&spi_done);
		status = msg->status;
		if (status == 0)
...
static int32_t spi_write_table(CODEC_SPI_CMD *cmds, int num)
{
	...
	tx = kcalloc(num*2, sizeof(uint8_t), GFP_KERNEL);
	...
	spi_transaction(&m);
	kfree(tx);
	return 0; //sp3d_spi_transaction(&m);
}
