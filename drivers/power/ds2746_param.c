...
static INT32 get_temp_01c(struct battery_type *battery)
{
	...
	struct battery_parameter* batt_param = container_of(
		battery, struct poweralg_type, battery)->pdata->batt_param;
	UINT32* temp_tbl = batt_param->m_temp_map_tbl[battery->id_index];
	...
	while (current_index >= 0 && current_index < TEMP_NUM-1) {
		...
		UINT32 temp_min = temp_tbl[current_index];
		UINT32 temp_max = temp_tbl[current_index + 1];

		if ((temp_min < battery->temp_adc) && (battery->temp_adc <= temp_max)) {
...
static void __software_charge_counter_update(struct battery_type *battery, UINT32 ms)
{
	...
	battery->charge_counter_adc =
		...
		batt_param->acr_adc_to_mv_coef / batt_param->acr_adc_to_mv_resl;
}
...
INT32 get_dead_battery_voltage(UINT32** m_param_tbl, int batt_id) {
	UINT32* tbl = m_param_tbl[batt_id];
	int i = 0;
	while(1) {
		if(0 == tbl[i])
			break;
