...
static void ds2746_battery_work(struct work_struct *work)
{
	...
	if (poweralg.battery.is_prediction)
		ds2746_program_alarm(di, config.predict_timeout_sec);
	else
		ds2746_program_alarm(di, config.polling_time_normal_sec);
...
static int ds2746_suspend(struct device *dev)
{
	...
	if (poweralg.charging_source == CONNECT_TYPE_NONE) {
		local_irq_save(flags);
		ds2746_program_alarm(di, config.polling_time_extension1_sec);
		di->slow_poll = 1;
		local_irq_restore(flags);
	}
