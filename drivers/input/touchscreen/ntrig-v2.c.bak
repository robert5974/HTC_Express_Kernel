...
static bool analy_init(void)
{
	...
	pr_err("[ts_anaConf]Fn=%d, HAN=%d, VAN=%d\n", mapData->fn,  mapData->han, mapData->van);
	return true;
}
...
static bool analy_pen(void)
{
	...
	pr_err("[ts_ana]PenState:%d,Th:%d,Mag:%d,Pressure:%d,X:%d,Y:%d,Btn:%d\n",
...
static int ntrig_analy_ioctl(struct inode *inode, struct file *file, unsigned int cmd,
			     unsigned long arg)
{
	char buf[100];
	int err = 0, ret = 0;
	...
	case ANALY_PEN:
		pr_err("ANALY_PEN\n");
		if (analy_pen()) {
			pr_err("ANALY_PEN PenState:%d,Th:%d,Mag:%d,Pressure:%d,X:%d,Y:%d,Btn:%d\n", mapData->pen.pState, mapData->pen.pTh, mapData->pen.mag, mapData->pen.pPress,mapData->pen.posX, mapData->pen.posY,mapData->pen.pBtn);
	...
	if (size)
		pr_err("[ts]analysis ioctl copy_to_user Error (%ld)", size);
...
static void compatible_tp_report(struct input_dev *idev, void *event, int index, bool finger, int ontip)
{
	if ( ontip == 0 && finger ) {
	...
	if ( ontip == 0 && !finger ) {
			...
			input_report_key(idev, BTN_TOOL_RUBBER, 0);
			input_report_key(idev, BTN_STYLUS, 0);
			input_sync(idev);
	}
	if ( ontip != 0 ) {
		if (finger ) {
			...
			input_report_abs(idev, ABS_MT_WIDTH_MAJOR, (touch->posDx + touch->posDy)/70);
		   ...
		   if ( ontip == 1 ) {
			struct ntg_touch_Ptouch *touch = (struct ntg_touch_Ptouch *)event;
			input_report_abs(idev, ABS_X, touch->posX);
			input_report_abs(idev, ABS_Y, touch->posY);
			input_report_abs(idev, ABS_PRESSURE, touch->Pressure);
			input_report_key(idev, BTN_STYLUS, 0);
			input_report_key(idev, BTN_TOOL_PEN, 0);
		  }
		  if ( ( ontip == 3 ) || ( ontip == 7) || ( ontip == 0x13 ) ) {
			struct ntg_touch_Ptouch *touch = (struct ntg_touch_Ptouch *)event;
			// pr_err("[report pen] %d, %d, Pressure = %d, bit = %x \n",  (touch->posX)/9 , (touch->posY)/10 , touch->Pressure, ontip);
			input_report_abs(idev, ABS_X, touch->posX  );
			input_report_abs(idev, ABS_Y, touch->posY  );
			input_report_abs(idev, ABS_PRESSURE, touch->Pressure);
			input_report_key(idev, BTN_TOUCH, 1);
			...
			input_report_key(idev, BTN_STYLUS, 0);
			if (ontip == 0x07) {
			 input_report_key(idev, BTN_TOOL_RUBBER, 1);
			}
			if (ontip == 0x13) {
			 input_report_key(idev, BTN_STYLUS, 1);
			}
		  }
		  input_sync(idev);
		}
...
static void htc_tp_report(struct input_dev *idev, struct ntg_touch_Stouch *event, bool finger, bool ontip)
{
	if (!ontip) {
		...
		if (finger) {
			NtrTrackedFinger *touch = (NtrTrackedFinger *)event;
			input_report_abs(idev, ABS_MT_AMPLITUDE,  0x28 << 16 | ((touch->posDx + touch->posDy)/30));
		...
		if (fingers[i].IsRemoved) {
			...
			if (event_google_enable == 1) {
				...
				if (i < fingers_num - 1)
					...
					input_report_abs(input, ABS_MT_POSITION, (0x01 << 31) | (fingers[i].posX << 16) | (fingers[i].posY));
			}
		}
	}
	if (event_google_enable == 1)
		...
		if (((1 == fingers_num) && (fingers[0].IsRemoved)) || (0 == fingers_num)) {
			if (event_google_enable == 1) {
				...
				input_report_abs(input, ABS_MT_AMPLITUDE, 0x00);
				input_report_abs(input, ABS_MT_POSITION,  1<<31);
				if (TSLog & Dbg_L4)
					pr_info("[ts]Finger Leave\n");
			}
		}
	   ...
	   unsigned long arg)
{
	...
	case NTRIG_IOCTL_READ_DATA:
		...
		if (count_isr_from_resume == 2) {/*fisrt report FW ver then mt report after resume*/
			...
			pr_err("[ntrig MT] IOCTL_READ_DATA, after resume send data to lib: repID = 0x%x, Contact count(1 byte) = 0x%x",
				byte_muti_tp[0], byte_muti_tp[MTM_REPORT_SIZE-1]);
		}
		if (TSLog & Dbg_L3)
			pr_err("[ntrig MT] NTRIG_IOCTL_READ_DATA: repID = 0x%x, Contact count(1 byte) = 0x%x",
				byte_muti_tp[0], byte_muti_tp[MTM_REPORT_SIZE-1]);
...
{
	if (ntg->touchEvnt->header.type == 0x01 && ntg->touchEvnt->header.channel == 0x02) {
		if (event_google_enable == 1) {
			compatible_tp_report(ntg->input_dev, &ntg->touchEvnt->stouch, 0x00, true, ntg->touchEvnt->stouch.bit );
			input_sync(ntg->input_dev);
		} else
			htc_tp_report(ntg->input_dev, &ntg->touchEvnt->stouch, true, (ntg->touchEvnt->stouch.bit & 0x02));

	} else if (ntg->touchEvnt->header.type == 0x01 && ntg->touchEvnt->header.channel == 0x03) {
		if (TSLog & Dbg_L8)
			pr_info("[ts_Pen] %d, %d, bit = %d\n", ntg->touchEvnt->ptouch.posX, ntg->touchEvnt->ptouch.posY, ntg->touchEvnt->ptouch.bit);

		if (event_google_enable == 1) {
		  // if ( ntg->touchEvnt->ptouch.bit == 3 || ntg->touchEvnt->ptouch.bit == 7 || ntg->touchEvnt->ptouch.bit == 13 ) {
			// pr_err("[ts_Pen] %d, %d, bit = %x\n", ntg->touchEvnt->ptouch.posX, ntg->touchEvnt->ptouch.posY, ntg->touchEvnt->ptouch.bit);

			compatible_tp_report(ntg->input_devP, (struct ntg_touch_Stouch *)&ntg->touchEvnt->ptouch, 0x00, false, ntg->touchEvnt->ptouch.bit );
...
static int ntrig_frame_parser(struct ntg_tsData *ntg, char *buf)
{
	...
	if (buf[i] == 0xA5 && buf[i+1] == 0x5A &&
		...
		if (ntg->touchEvnt->header.channel == 0x30) {
			...
			case 0x02:
				memcpy(byte_muti_tp, &buf[nIndex+6], MTM_REPORT_SIZE+4);
				pr_err("[ts_MT]State= %#x,Fgr-idx= %#x,%#x;x= %#x,%#x;y=%#x,%#x;dx=%#x,%#x;dy=%#x,%#x,VDef= %#x,%#x,%#x,%#x\n",
					byte_muti_tp[3+(i*15)], byte_muti_tp[4+(i*15)], byte_muti_tp[5+(i*15)], byte_muti_tp[6+(i*15)],
					byte_muti_tp[7+(i*15)], byte_muti_tp[8+(i*15)], byte_muti_tp[9+(i*15)], byte_muti_tp[10+(i*15)],
					byte_muti_tp[11+(i*15)], byte_muti_tp[12+(i*15)], byte_muti_tp[13+(i*15)], byte_muti_tp[14+(i*15)],
					byte_muti_tp[15+(i*15)], byte_muti_tp[16+(i*15)], byte_muti_tp[17+(i*15)]);
			...
			case 0x03:
				...
				*/
				...
				if (fCheckSum) {
					...
					ntg->touchEvnt->ptouch.Pressure=byte_muti_tp[6+(i*15)];
				} else
...
static int ntrig_ts_probe(struct spi_device *dev)
{
	...
	ntg_ts->input_devP->name = "Ntrig-Pen-touchscreen-v2";
	...
	input_set_abs_params(ntg_ts->input_dev, ABS_MT_POSITION_X, ntg_ts->abs_x_min, ntg_ts->abs_x_max, 0, 0);
	input_set_abs_params(ntg_ts->input_dev, ABS_MT_POSITION_Y, ntg_ts->abs_y_min, ntg_ts->abs_y_max, 0, 0);
	input_set_abs_params(ntg_ts->input_dev, ABS_MT_TOUCH_MAJOR, ntg_ts->abs_pressure_min, ntg_ts->abs_pressure_max, 0, 0);
	input_set_abs_params(ntg_ts->input_dev, ABS_MT_WIDTH_MAJOR, ntg_ts->abs_width_min, ntg_ts->abs_width_max, 0, 0);
	input_set_abs_params(ntg_ts->input_dev, ABS_MT_TRACKING_ID, 0, ABS_MT_TRACKING_ID_MAX, 0, 0);

        input_set_abs_params(ntg_ts->input_dev, ABS_MT_AMPLITUDE, 0, ((255 << 16) | 30), 0, 0);
        input_set_abs_params(ntg_ts->input_dev, ABS_MT_POSITION, 0, ((1 << 31) | (ntg_ts->abs_x_max << 16) | ntg_ts->abs_y_max), 0, 0);


	set_bit(EV_ABS, ntg_ts->input_devP->evbit);
	set_bit(EV_KEY, ntg_ts->input_devP->evbit);
	set_bit(ABS_X, ntg_ts->input_devP->absbit);
	set_bit(ABS_Y, ntg_ts->input_devP->absbit);
	set_bit(ABS_PRESSURE, ntg_ts->input_devP->absbit);
	set_bit(BTN_TOUCH, ntg_ts->input_devP->keybit);
	set_bit(BTN_TOOL_PEN, ntg_ts->input_devP->keybit);
	set_bit(BTN_TOOL_RUBBER, ntg_ts->input_devP->keybit);
	set_bit(BTN_STYLUS, ntg_ts->input_devP->keybit);
	set_bit(KEY_UNKNOWN, ntg_ts->input_devP->keybit);

	input_set_abs_params(ntg_ts->input_devP, ABS_X, ntg_ts->abs_x_min, ntg_ts->abs_x_max, 4, 0);
	input_set_abs_params(ntg_ts->input_devP, ABS_Y, ntg_ts->abs_y_min, ntg_ts->abs_y_max, 4, 0);
 	input_set_abs_params(ntg_ts->input_devP, ABS_PRESSURE, 0, ntg_ts->abs_pressure_max,0,0);
