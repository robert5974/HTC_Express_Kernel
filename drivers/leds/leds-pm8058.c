					   ...
					   enum led_brightness brightness)
{
	...
	pwm_duty = (brightness * 100) / LED_FULL;

	printk(KERN_INFO "pwm_duty: %d, current_pwm_duty:%d\n", pwm_duty, ldata->current_pwm_duty);
	if (ldata->current_pwm_duty == pwm_duty)
		ldata->current_pwm_duty = pwm_duty;
	else {
		pwm_duty_gap = pwm_duty - ldata->current_pwm_duty;
		duties[ldata->duites_size - 1] = pwm_duty;
		if (abs(pwm_duty_gap)/(ldata->duites_size - 1) <= 1) {
			if (brightness) {
				for (index = 0; index < ldata->duites_size - 1; index++) {
					if (index > abs(pwm_duty_gap)) {
						duties[index] = ldata->current_pwm_duty + pwm_duty_gap;
					} else {
						if (pwm_duty > ldata->current_pwm_duty)
							duties[index] = ldata->current_pwm_duty + index;
						else
							duties[index] = ldata->current_pwm_duty - index;
					}
				}
				...
				for (index = 0; index < ldata->duites_size - 1; index++) {
					if (index > abs(pwm_duty_gap)) {
						duties[ldata->duites_size + index] = ldata->current_pwm_duty + pwm_duty_gap;
					} else {
						if (pwm_duty > ldata->current_pwm_duty)
							duties[ldata->duites_size + index] = ldata->current_pwm_duty + index;
						else
							duties[ldata->duites_size + index] = ldata->current_pwm_duty - index;
					}
				}
			...
			if (brightness) {
				for (index = 0; index < ldata->duites_size - 1; index++) {
					duties[index] = ldata->current_pwm_duty + (pwm_duty_gap/(ldata->duites_size - 1)) * index;
				}
			} else {
				for (index = 0; index < ldata->duites_size - 1; index++) {
					duties[ldata->duites_size + index] = ldata->current_pwm_duty + (pwm_duty_gap/(ldata->duites_size - 1)) * index;
				}
			}
		}
		ldata->current_pwm_duty = pwm_duty;
	...
	if (brightness) {
		pm8058_pwm_config_led(ldata->pwm_led, id, mode, ldata->out_current);
		...
		if (ldata->flags & PM8058_LED_LTU_EN) {
			pduties = &duties[ldata->duites_size];
			pm8058_pwm_lut_config(ldata->pwm_led,
					      ldata->period_us,
					      pduties,
					      ldata->duty_time_ms,
			...
			queue_delayed_work(g_led_work_queue,
					   ...
					   msecs_to_jiffies(ldata->duites_size * ldata->duty_time_ms));
