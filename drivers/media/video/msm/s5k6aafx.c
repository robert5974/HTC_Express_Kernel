...
static int s5k6aafx_set_sensor_mode(int mode, enum qtr_size_mode qtr_size_mode_value)
{
	...
	if (config_csi == 0) {
		if (sinfo->csi_if) {
			...
			s5k6aafx_csi_params.settle_cnt = 0x14;
...
int s5k6aafx_sensor_config(void __user *argp)
{
	...
	case CFG_SET_QTR_SIZE_MODE:
        ...
        case CFG_I2C_IOCTL_R_OTP:
                ...
                if (copy_to_user(argp, s5k6aafx_pdev->dev.platform_data, sizeof(struct msm_camera_sensor_info))){
			pr_info("[CAM] YNC failed to set sensor info");
                }
		rc = 0;
                break;
#if 0
	...
	default:
		//YNC
		pr_info("[CAM] sensor cmd %d",  cfg_data.cfgtype);
		rc = -EINVAL;
		break;
	}
...
probe_failure:
	if (s5k6aafx_sensorw) kfree(s5k6aafx_sensorw);
