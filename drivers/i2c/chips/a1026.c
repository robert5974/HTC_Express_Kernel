...
static void a1026_gpio_set_value(int gpio, int value)
{
	if (gpio >  0)
...
static ssize_t a1026_bootup_init(struct file *file, struct a1026img *img)
{
	...
	while (retry--) {
		/* Reset A1026 chip */
		a1026_gpio_set_value(pdata->gpio_a1026_reset, 0);

		/* Enable A1026 clock */
		a1026_gpio_set_value(pdata->gpio_a1026_clk, 1);
		mdelay(1);

		/* Take out of reset */
		a1026_gpio_set_value(pdata->gpio_a1026_reset, 1);
...
static ssize_t chk_wakeup_a1026(void)
{
	...
	if (a1026_suspended == 1) {
		...
		a1026_gpio_set_value(pdata->gpio_a1026_wakeup, 0);
		msleep(120);
		...
		a1026_gpio_set_value(pdata->gpio_a1026_wakeup, 1);
		if (rc < 0) {
			pr_err("%s: failed (%d)\n", __func__, rc);
...
int a1026_set_config(int newid, int mode)
{
	...
	case A1026_PATH_INCALL_RECEIVER:
		a1026_gpio_set_value(pdata->gpio_a1026_micsel, 0);
		i2c_cmds = phonecall_receiver;
		size = sizeof(phonecall_receiver);
		break;
	case A1026_PATH_INCALL_HEADSET:
		a1026_gpio_set_value(pdata->gpio_a1026_micsel, 1);
	   ...
	   unsigned long arg)
{
	...
	case A1026_SET_MICSEL_ONOFF:
		...
		if (copy_from_user(&mic_sel, argp, sizeof(mic_sel)))
			return -EFAULT;
		a1026_gpio_set_value(pdata->gpio_a1026_micsel, !!mic_sel);
		rc = 0;
		break;
