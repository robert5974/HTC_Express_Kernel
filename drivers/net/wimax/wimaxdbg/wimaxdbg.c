...
ssize_t wimaxdbg_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
    ...
    else if (dbg_para == 14) { // 14: Disable SDIO HW RESET
    ...
    else if (dbg_para == 22) { // 22: Disable SDIO CLK to high speed (workaround) - 49152000
        printk(KERN_INFO "%s: mmc_wimax_set_sdcclk_highspeed:0\n", __func__);
        mmc_wimax_set_sdcclk_highspeed(0);
    }
    else if (dbg_para == 23) { // 23: Enable SDIO CLK to high speed (workaround)
	...
	/* 38: Set mmc_wimax_set_disable_irq_config 0 , No disable IRQ feature */
	else if (dbg_para == 38) {
	printk(KERN_INFO "%s: mmc_wimax_set_disable_irq_config:0\n", __func__);
	mmc_wimax_set_disable_irq_config(0);
    }
	/* 39: Set mmc_wimax_set_disable_irq_config 1 , Disable irq when system suspend */
	else if (dbg_para == 39) {
	printk(KERN_INFO "%s: mmc_wimax_set_disable_irq_config:1\n", __func__);
	mmc_wimax_set_disable_irq_config(1);
    }
	/* 40: Set mmc_wimax_set_disable_irq_config 2 , Disable IRQ when WiMAX CLK off */
	else if (dbg_para == 40) {
	printk(KERN_INFO "%s: mmc_wimax_set_disable_irq_config:2\n", __func__);
	mmc_wimax_set_disable_irq_config(2);
    }
    else {
