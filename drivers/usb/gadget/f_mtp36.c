...
//#else
//static const char shortname[] = "android_mtp_tunnel";
//#endif
...
static int
mtp36_function_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	dev->cdev = cdev;
	DBG(cdev, "mtp36_function_bind dev: %p\n", dev);
...
static void
mtp36_function_unbind(struct usb_configuration *c, struct usb_function *f)
{
	struct mtp_dev	*dev = func_to_dev(f);
	struct usb_request *req;
	int i;
...
static void
mtp36_function_release(struct usb_configuration *c, struct usb_function *f)
{
	struct mtp_dev	*dev = func_to_dev(f);
	struct usb_request *req;
	int i;
...
static int mtp36_function_setup(struct usb_function *f,
					const struct usb_ctrlrequest *ctrl)
{
	...
	VDBG(cdev, "mtp36_function_setup "
...
static int mtp36_function_set_alt(struct usb_function *f,
		unsigned intf, unsigned alt)
{
	...
	DBG(cdev, "mtp36_function_set_alt intf: %d alt: %d\n", intf, alt);
...
static void mtp36_function_disable(struct usb_function *f)
{
	struct mtp_dev	*dev = func_to_dev(f);
	struct usb_composite_dev	*cdev = dev->cdev;

	DBG(cdev, "%s mtp36_function_disable\n", dev->function.name);
	dev->state = STATE_OFFLINE;
	usb_ep_disable(dev->ep_in);
	usb_ep_disable(dev->ep_out);
...
static int mtp36_bind_config(struct usb_configuration *c)
{
	struct mtp_dev *dev;
	int ret = -ENOMEM;

	printk(KERN_INFO "mtp36_bind_config\n");
	...
	dev->function.descriptors = fs_mtp_descs;
	dev->function.hs_descriptors = hs_mtp_descs;
	dev->function.bind = mtp36_function_bind;
	dev->function.unbind = mtp36_function_unbind;
	dev->function.setup = mtp36_function_setup;
	dev->function.set_alt = mtp36_function_set_alt;
	dev->function.disable = mtp36_function_disable;
#ifdef CONFIG_USB_GADGET_DYNAMIC_ENDPOINT
	dev->function.release = mtp36_function_release;
...
static struct android_usb_function mtp_function = {
	...
	.name = "mtp",
#endif
	.bind_config = mtp36_bind_config,
};

static int __init init(void)
{
	printk(KERN_INFO "f_mtp36 init\n");
