...
int mmc_wimax_uart_switch(int uart)
{
	...
	if (uart == 0) { /* Enable CPU wimax UART without microUSB */
		...
		printk(KERN_INFO "[WIMAX] %s config wimax uart gpio on\n", __func__);
		config_gpio_table(wimax_uart_on_gpio_table,
			  ARRAY_SIZE(wimax_uart_on_gpio_table));	
	} else if (uart == 9) {
		printk(KERN_INFO "[WIMAX] %s config wimax uart gpio off\n", __func__);
		...
		printk(KERN_INFO "[WIMAX] %s: wimax_uart_switch %d\n", __func__, wimax_uart_switch);
		...
		if (ret != 0)
		printk(KERN_INFO "[WIMAX] %s: wimax_uart_switch ret=%d\n", __func__, ret);
	}
#endif
	return 0;
}
